...
exports.shows = function (req,res){
    var transactionId = req.params.transaction_id;   
    sequelize.query(`SELECT transactionId, t.listingId, seller.id sellerId, buyerId, t.createdAt, t.updatedAt, 
    t.status, offer, il.servicetitle, imageName,seller.username sellerUser, buyer.username buyerUser, buyerReady, sellerReady, 
    paymentMethod, paymentId 
    FROM ServicesTransactions t 
    INNER JOIN servicelists il ON t.listingId = il.serviceid 
    INNER JOIN Users seller ON seller.id = il.user_id 
    INNER JOIN Users buyer ON buyer.id = buyerId 
    WHERE t.transactionId = ` + transactionId, { type: sequelize.QueryTypes.SELECT }).then((Transactions) => {
        console.log(Transactions[0]);
